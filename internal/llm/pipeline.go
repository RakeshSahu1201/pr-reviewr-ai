...
// Used by components (e.g. Worker) that need to call individual LLM helpers
// such as BuildContextFromDiff without running the full pipeline.
func (p *Pipeline) Client() LLMClient { return p.client }
...
func (p *Pipeline) Run(ctx context.Context, diff string) (*AnalysisResult, error) {
	...
	// ─────────────────────────────────────────────────────────
	// Stage 1: Context Retrieval (RAG — five-optimisation path)
	// ParseDiffFiles → ExtractImports → truncate → intersect → LLM
	// ─────────────────────────────────────────────────────────
	if ctx.Err() == nil {
		ctxResult, err := BuildContextFromDiff(ctx, p.client, diff)
		if err == nil {
